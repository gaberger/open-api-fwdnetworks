defmodule FwdClient.Plugin do
  ...
  **stack**. To modify the behavior of the client, applications can add or modify plugins.
  ...
  Plugins are functions that accept a `FwdClient.Operation` struct — and any options — and return
  either an `:ok` or an `:error` tuple:

      alias FwdClient.{Error, Operation}
