if Code.ensure_loaded?(:opentelemetry) do
  defmodule FwdClient.Plugin.OpenTelemetry do
    @moduledoc """
    OpenTelemetry bindings for all operation requests

    This module provides an easy way to report FwdClient operations to OpenTelemetry. The library
    has built-in support for Erlang telemetry in the default client `FwdClient.Client`. This module
    ...
    your main `Application` module start:
        ...
        def start(_type, _args) do
          FwdClient.Plugin.OpenTelemetry.setup()
    ...
    def setup(_opts \\ []) do
      ...
      :telemetry.attach(
        ...
        [:oapi_FwdClient, :request, :exception],
